...
export function findPersonalSchedules(id) {
  ...
  return axios
    .get(`${baseURL}/v1/personalSchedules?id=${personId}`)
    .then(function(res) {
      if (res.status !== 200)
...
export function findSingleSchedule(scheduleId) {
  ...
  return axios
    .get(`${baseURL}/v1/schedule?id=${scheduleId}`)
    .then(function(res) {
      if (res.status !== 200)
        throw new Error('bad response from server' + res.status);
      return res.data;
...
export function removeSchedule(scheduleId) {
  scheduleId = scheduleId.replace(/['"]+/g, '');
  return axios
    .get(`${baseURL}/v1/removeSchedule?id=${scheduleId}`)
    .then(function(res) {
      if (res.status !== 200)
        throw new Error('bad response from server' + res.status);
      return res.data;
...
export function addSchedule(personId, name) {
  ...
  return axios
    .get(`${baseURL}/v1/addSchedule?personId=${personId}&name=${name}`)
    .then(function(res) {
      if (res.status !== 200)
        throw new Error('bad response from server' + res.status);
      return res.data;
